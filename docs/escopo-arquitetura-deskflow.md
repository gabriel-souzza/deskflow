...
| **Application Layer** | - Casos de uso: `CreateBookingUseCase`, `CheckInUseCase`, `ReleaseExpiredBookingsUseCase`, `ConsumeQuotaUseCase` <br>- Orquestram repositórios e publicam eventos de domínio | - TanStack Query (`useQuery`, `useMutation`) para carregar e mutar dados <br>- Server‑Sent Events (`useEffect` + `EventSource`) para receber atualizações de disponibilidade em tempo real <br>- Zustand (opcional) para estado UI local (filtros, modal abertos) |
| **Domain Layer** | - Entidades/Value Objects: `Booking`, `TimeSlot`, `Capacity`, `AvailabilityWindow`, `QuotaPeriod` (arquivo puro `dataclasses`) <br>- Regras de negócio: `TimeSlot.overlaps()`, `AvailabilityWindow.with_booking()`, `QuotaPeriod.consume()` <br>- Exceções de domínio (`CapacityExceededError`, `QuotaExceededError`, `InvalidTimeSlotError`) | - Tipos TypeScript gerados a partir de Zod schemas (mesmas regras de validação) <br>- Funções puras (`isAlignedToGrid`, `overlaps`) reutilizadas em validação de formulário <br>- Nenhuma lógica de negócio que dependa de React ou Next.js |
| **Infrastructure Layer** | - Repositórios SQLAlchemy 2.0 mapeando para tabelas PostgreSQL (`bookings`, `availability_windows`, `quota_periods`) <br>- Migrations Alembic <br>- Publicador de eventos (Redis Streams ou Postgres NOTIFY) <br>- Serviço de geração de QR Code (bibliotecas `qrcode`, `pilow`) <br>- Clock e IdGenerator inseríveis (facilita testes) | - Biblioteca HTTP (fetch ou axios) wrappers para endpoints da API <br>- Biblioteca de QR Code para leitura no móvil (`react-qr-reader` ou equivalente) <br>- Tailwind CSS para estilização utilitária <br>- shadcn/ui como base de componentes acessíveis |
...
|---|---|---|
| **Colisão de Horários** (não pode haver duas reservas confirmadas que se sobreponham no mesmo recurso) | **Domain** – método `AvailabilityWindow.with_booking()` verifica `if booking_id in self.confirmed_booking_ids` e capacidade. <br>**Application** – `CreateBookingUseCase` chama `window.with_booking()` para cada slot da reserva antes de salvar. | Se a janela já estiver cheia ou o ID já existir, lança `CapacityExceededError` ou `DuplicateBookingError`. O caso de uso não persiste até que todas as janelas passem na validação. |
...
- Assegura o **controle de custos** por departamento: toda reserva confirmada consome quota; se o limite for atingido, novas reservas são bloqueadas até o próximo período ou até que alguma seja cancelada (que devolve quota via `QuotaPeriod.without_consumption` – método análogo a `without_booking`).
...
Todas as funcionalidades listadas são **estritamente necessárias** para atender aos requisitos obrigatórios (RO-01 a RO-08) e para colocar em prática os três invariantes do domínio. Nenhum componente adiciona custo de licença ou infraestrutura além da stack já definida.
